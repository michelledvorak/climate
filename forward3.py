## this is Chris Smith's forward.py module modified slightly to use the Held 2-layer ocean model (forc_to_temp2)
...
from .ancil import natural, natural_2, cmip6_volcanic, cmip6_solar, historical_scaling
...
def forc_to_temp2(t0, tdeep0, lam, eps, gam, f, Cml, Cdeep, dt, e=1.0):
    """Calculate temperature from a given radiative forcing using two-layer model.

    Inputs:
        t0: Temperature in timestep t-1
        tdeep0: Deep ocean temperature in timestep t-1
    ...
    Outputs:
        t1: surface layer temperature in timestep t 
        tdeep: deep ocean temperature in timestep t 
    """
    
    t1 = t0 + ((-lam*t0 - eps*gam*(t0 - tdeep0)) + np.sum(f*e))*dt/Cml
    tdeep = tdeep0 + gam*(t0 - tdeep0)*dt/Cdeep
    TOA_imbalance = np.sum(f*e) - lam*t1-(eps-1)*(gam*(t1-tdeep))
        
    return t1, tdeep, TOA_imbalance
...
def fair_scm(
    ...
    F = np.zeros((nt, nF))
    C_acc = np.zeros(nt)
    T_s = np.zeros(thermal_boxes_shape) #for restart_in=False, this creates initial temperature anomaly according to FaIR
    T_j = np.zeros(nt)
    T_d = np.zeros(nt)
    N = np.ones(nt)
    T = np.zeros(nt)
    ...
    if restart_in:
        ...
        T[0], T_d[0], N[0] = forc_to_temp2(T_j_minus1, T_d_minus1, lam, eps, gam, F[0,:], Cml, Cdeep, dt) #T_d_minus1 not yet defined!
    ...
    if restart_in == False:
        ...
        # T_s[0,:] = (q[0,:]/d)*(np.sum(F[0,:]))
        # T[0] = np.sum(T_s[0,:], axis=-1)
        T[0] = 0
       
        # Initialize net radiative balance
        # N[0] = F2x
        N[0] = 0
    ...
    for t in range(1,nt):
        ...
        if emissions_driven:
            if useMultigas:
                ...
                T[t], T_d[t], N[t] = forc_to_temp2(
                  T[t-1], T_d[t-1], lam, eps, gam, F[t,:], Cml, Cdeep, dt, e=efficacy)
                ...
                T[t], T_d[t], N[t] = forc_to_temp2(T[t-1], T_d[t-1], lam, eps, gam, F[t,:], Cml, Cdeep, dt)
